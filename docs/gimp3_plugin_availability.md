...
| **D: 非対話不可** | UI 必須、またはシリアライズデータ依存で NONINTERACTIVE 不可 |

| プラグイン | 区分 | 理由 |
|-----------|-----|------|
| crop-zealous | A | 引数なし |
| gradient-map | A | 引数なし（前景色グラデーション使用） |
| guillotine | A | 引数なし（ガイドで分割、フィルター用途外） |
| wavelet-decompose | A | 3 引数、全 scalar |
...
### plug-in-align-layers（区分 A）

| GIMP 引数 | 型 | 範囲 | デフォルト | CSP UI | 値 |
|---------|----|----|---------|------|----|
| `horizontal-style` | GimpChoice | none/collect/fill-left-to-right/fill-right-to-left/snap-to-grid | none | 固定 | `"none"` |
| `horizontal-base` | GimpChoice | left-edge/center/right-edge | left-edge | 固定 | `"left-edge"` |
| `vertical-style` | GimpChoice | none/collect/fill-left-to-right/fill-right-to-left/snap-to-grid | none | 固定 | `"collect"` |
| `vertical-base` | GimpChoice | top-edge/center/bottom-edge | top-edge | 固定 | `"center"` |
| `grid-size` | gint | 1–200 | 10 | スライダー公開 | UI 値 |
| `ignore-bottom-layer` | gboolean | TRUE/FALSE | TRUE | 固定 | `1`（TRUE） |
| `use-bottom-layer` | gboolean | TRUE/FALSE | FALSE | 固定 | `0`（FALSE） |
...
### plug-in-decompose（区分 A）

| GIMP 引数 | 型 | 範囲 | デフォルト | CSP UI | 値 |
|---------|----|----|---------|------|----|
| `decompose-type` | GimpChoice | rgb/rgba/alpha/hsv/hsl/cmyk/lab/lch/ycbcr470/ycbcr709/ycbcr470f/ycbcr709f | rgb | 固定 | `"rgb"` |
| `layers-mode` | gboolean | TRUE/FALSE | TRUE | 固定 | `1`（TRUE） |
| `use-registration` | gboolean | TRUE/FALSE | FALSE | 固定 | `0`（FALSE） |
...
### plug-in-guillotine（区分 A）

引数なし。ガイドに沿って画像を分割する操作のため、単体フィルターとしての用途は限定的。
...
### plug-in-smooth-palette（区分 A）

| GIMP 引数 | 型 | 範囲 | デフォルト | CSP UI | 値 |
|---------|----|----|---------|------|----|
| `width` | gint | 2–MAXSIZE | 256 | 固定 | `256` |
| `height` | gint | 2–MAXSIZE | 64 | 固定 | `64` |
| `n-tries` | gint | 1–1024 | 50 | スライダー公開 | UI 値 |
| `show-image` | gboolean | TRUE/FALSE | TRUE | 固定 | `0`（FALSE） |

> 画像内の色からスムーズなパレット画像を生成する。フィルターとしての用途は限定的。
...
### plug-in-wavelet-decompose（区分 A）

| GIMP 引数 | 型 | 範囲 | デフォルト | CSP UI | 値 |
|---------|----|----|---------|------|----|
| `scales` | gint | 1–7 | 5 | スライダー公開 | UI 値 |
| `create-group` | gboolean | TRUE/FALSE | TRUE | 固定 | `1`（TRUE） |
| `create-masks` | gboolean | TRUE/FALSE | FALSE | 固定 | `0`（FALSE） |
...
| 4 | wavelet-decompose | `plug-in-wavelet-decompose` | ウェーブレット分解、引数 3 個 |
| 5 | jigsaw | `plug-in-jigsaw` | ジグソーテクスチャ、引数 5 個 |
| 6 | hot | `plug-in-hot` | NTSC/PAL セーフカラー補正、引数 3 個 |
| 7 | sparkle | `plug-in-sparkle` | きらめきエフェクト、引数 13 個 |
| 8 | border-average | `plug-in-border-average` | 境界色平均化、引数 2 個 |
| 9 | tile | `plug-in-tile` | タイル繰り返し、引数 3 個 |
| 10 | tile-small | `plug-in-tile-small` | 小タイル、引数 1 個 |
| 11 | pagecurl | `plug-in-pagecurl` | ページめくり効果、引数 5 個 |
| 12 | fractal-explorer | `plug-in-fractal-explorer` | フラクタル生成、引数多（区分 B） |
