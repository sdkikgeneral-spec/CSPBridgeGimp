...
2. 各パスを走査して *.exe を列挙
3. 各 EXE に対して Wire Protocol クエリフェーズを実行：
   a. 匿名パイプ（stdin/stdout）を作成
   b. 環境変数をセット（GIMP_PLUGIN_FD 等）
   c. 子プロセス起動
   d. GP_QUERY 送信
   e. GP_PROC_INSTALL を受信 → メタデータ収集
   f. GP_QUIT 送信、プロセス終了待ち
4. tkinter チェックリストダイアログで一覧表示
5. ユーザーがチェックを入れたプラグインのみ plugins.json に書き出し
```

### 5.3 Wire Protocol クエリフェーズで使うメッセージ

| メッセージ | 方向 | 役割 |
|---|---|---|
| `GP_QUERY` | スキャナー → プラグイン | クエリ開始 |
| `GP_PROC_INSTALL` | プラグイン → スキャナー | メタデータ受信 |
| `GP_QUIT` | スキャナー → プラグイン | 終了 |
...
1. `tools/scanner/scan_and_select.py` — Wire Protocol クエリフェーズ（GP_QUERY / GP_PROC_INSTALL / GP_QUIT）
